...
subset_ds = subset_data(full_train_ds, int(argv[2]))
...
class VGG16(pl.LightningModule):
  def __init__(self):
    ...
    self.model.conv1 = Conv2d(3, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
...
model = VGG16()
trainer = pl.Trainer(max_epochs=int(argv[1]), logger=logger)
...
inference_model = VGG16.load_from_checkpoint(script_name + '_epochs=' + str(argv[1]) + '_k=' + str(argv[2]) + ".pt", map_location=device)
...
with open('./text_logs/' + script_name + '_epochs=' + str(argv[1]) + '_k=' + str(argv[2]) + '_.txt', 'w') as f:
